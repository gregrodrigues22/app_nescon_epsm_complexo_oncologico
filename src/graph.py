...
def _wrap_label(s: str, max_chars: int = 22) -> str:
    s = str(s)
    if len(s) <= max_chars:
        return s
...
def normalize_lonlat(series: pd.Series) -> pd.Series:
...
def map_points(df: pd.DataFrame, lon_col: str, lat_col: str, text_col: str | None = None):
    ...
    df[lon_col] = normalize_lonlat(df[lon_col])
    df[lat_col] = normalize_lonlat(df[lat_col])
    df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=[lon_col, lat_col])
    df = df[(df[lon_col].between(-180, 180)) & (df[lat_col].between(-90, 90))]
    if df.empty:
        return go.Figure()

    c_lat = float(df[lat_col].mean())
    c_lon = float(df[lon_col].mean())
    hovertext = df[text_col] if (text_col and text_col in df.columns) else None
